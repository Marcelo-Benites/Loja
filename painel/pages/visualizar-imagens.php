...
<div class="box-content">
	...
	<?php
	   if(isset($_GET['deletar'])){
	   	...
	   	$id = (int)$_GET['deletar'];
	   	$imagens = MySql::conectar()->prepare("SELECT * FROM `tb_site.moto` WHERE moto_id = $id");
	   	$imagens->execute();
	   	$imagens = $imagens->fetchAll();
	   	foreach ($imagens as $key => $value) {
	   		@unlink(BASE_DIR_PAINEL.'/uploads/'.$value['imagem']);
	   	}
	   	MySql::conectar()->exec("DELETE FROM `tb_site.moto` WHERE moto_id = $id");
	   	MySql::conectar()->exec("DELETE FROM `tb_text.moto` WHERE id = $id");
	...
	if(isset($_POST['acao']) && $_POST['acao'] == 'Buscar!'){
	 ...
	 $sql = MySql::conectar()->prepare("SELECT * FROM `tb_text.moto` $query");
	 $sql->execute();
	 $fotos = $sql->fetchAll();
	 if($query != ''){
	 ...
	 foreach ($fotos as $key => $value) {
	  $imagemSingle = MySql::conectar()->prepare("SELECT * FROM `tb_site.moto` WHERE moto_id = $value[id] LIMIT 1");
		...
		<div  class="box-single-wraper">
			...
			<div class="body-box" style="text-align: center ;" >
					...
					<div style="border-top: 2px solid rgb(210,210,210); padding: 10px;"class="group-btn">
						<a class="btn delete"  href="<?php echo INCLUDE_PATH_PAINEL ?>visualizar-imagens?deletar=<?php echo $value['id'] ?>"; ?><i class="fa fa-times"></i> Excluir</a>
