<?php
	$id = (int)$_GET['id'];
	$sql = MySql::conectar()->prepare("SELECT * FROM `tb_text.carro` WHERE id = ?");
	...
	$pegaImagens = MySql::conectar()->prepare("SELECT * FROM `tb_site.carro` WHERE carro_id = $id");
	$pegaImagens->execute();
...
<div class="box-content">
	...
	if(isset($_GET['deletarImagem'])){
		...
		MySql::conectar()->exec("DELETE FROM `tb_site.carro` WHERE imagem = '$idImagem'");
...
<div class="box-content">
   ...
   <div class="card-title">Informações Dos Carros</div>
			 	...
			 	if($_FILES['imagem']['name'][0] != ''){
					for($i =0; $i < $amountFiles; $i++){
						...
						foreach ($imagens as $key => $value) {
							MySql::conectar()->exec("INSERT INTO `tb_site.carro` VALUES (null,$id,'$value')");
						}

			    }

			    $sql = MySql::conectar()->prepare("UPDATE `tb_text.carro` SET nome = ?, descricao = ? WHERE id = $id");
...
<div class="card-title">Imagens Dos Carros</div>
      <div class="boxes">
      	<?php
	...
	<div class="box-single-wraper">
			...
			<div style="text-align: center;" class="group-btn">
				<a class="btn delete" href="<?php echo INCLUDE_PATH_PAINEL ?>editar-imagem1?id=<?php echo $id; ?>&deletarImagem=<?php echo $value['imagem'] ?>"><i class="fa fa-times"></i> Excluir</a>
			</div><!--group-btn-->
		</div>
	</div>		
   <?php } ?>
